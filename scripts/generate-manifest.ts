...
function rgbToHex(r: string | number, g: string | number, b: string | number): string {
    return (
        "#" +
        [r, g, b]
            .map((x) => {
...
function extractCSSColors(): Colors {
    ...
    try {
        ...
        const base7Match = cssContent.match(/--oui-color-base-7:\s*(\d+)\s+(\d+)\s+(\d+)/);
        const primaryMatch = cssContent.match(/--oui-color-primary:\s*(\d+)\s+(\d+)\s+(\d+)/);

        const backgroundColor = base7Match ? rgbToHex(base7Match[1], base7Match[2], base7Match[3]) : "#000000";

        const themeColor = primaryMatch ? rgbToHex(primaryMatch[1], primaryMatch[2], primaryMatch[3]) : "#000000";
...
function generateManifest() {
    ...
    const manifest = {
        name: config.VITE_APP_NAME || "Dead y00ts DEX",
        short_name: config.VITE_APP_NAME || "Dead y00ts DEX",
        description: config.VITE_APP_DESCRIPTION || "A perpetual trading DEX by Dead y00ts NFT on Solana",
        start_url: startUrl,
        scope: basePath,
        display: "standalone",
